...
async function main() {
  ...
  console.log('Total calculation questions:', questions.length);

  // Build CSV
  const headers = ['ID', 'Category', 'Difficulty', 'Question', 'Option A', 'Option B', 'Option C', 'Option D', 'Option E', 'Correct Answer', 'Explanation', 'Status'];
  ...
  for (const q of questions) {
    ...
    const row = [
      escapeCSV(q.id),
      escapeCSV(q.question_categories?.name || ''),
  ...
  const csv = rows.join('\n');
  const outputPath = path.join(__dirname, '..', 'calculations-questions.csv');
